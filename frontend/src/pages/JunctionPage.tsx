...
import { VehicleInput, SignalTimer } from '../types';
...
const VEHICLE_TYPES: VehicleType[] = [
  ...
  { key: 'bikes',         label: 'Bike',          icon: '🏍', pcu: 0.75 },
  ...
  { key: 'cycles',        label: 'Cycle',         icon: '🚲', pcu: 0.5  },
];

const emptyVehicleInput = (): Omit<VehicleInput, 'direction'> => ({
  cars: 0, bikes: 0, buses: 0, mini_trucks: 0, medium_trucks: 0, big_trucks: 0, cycles: 0,
});

function calcPCU(input: Omit<VehicleInput, 'direction'>): number {
  return VEHICLE_TYPES.reduce((sum, vt) => sum + (input[vt.key] || 0) * vt.pcu, 0);
}
...
export default function JunctionPage() {
  ...
  const getActiveDirection = () =>
    Object.entries(signals).find(([, s]) => s.phase === 'green')?.[0] || 'N';

  const getCycleLength = () =>
    timers.reduce((sum, t) => sum + t.green_time + t.yellow_time, 0);
  ...
  const getTimer = (dir: string): SignalTimer =>
    ...
    { id: 0, junction_id: 0, direction: dir, green_time: 30, yellow_time: 5, red_time: 90 };
  ...
  const chartData = optimizationResult
    ? DIRECTIONS.map(dir => ({
        direction: dir,
        'Old Green': optimizationResult.oldTimers.find(t => t.direction === dir)?.green_time || 30,
        'New Green': optimizationResult.newTimers.find(t => t.direction === dir)?.green_time || 30,
      }))
    : [];
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: '#f0ebe0', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{
            fontFamily: 'Orbitron', fontSize: '1.05rem', fontWeight: 900,
            color: '#f0e8d0', margin: 0, letterSpacing: 3,
          }}>
      ...
      <div style={{ flex: 1, display: 'flex', gap: 14, padding: 14, overflow: 'auto' }}>
        ...
        <div
          ...
          style={{ width: '40%', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}
        >
            ...
            {DIRECTIONS.map(dir => {
              ...
              return (
                <SignalLight
                  key={dir}
                  direction={dir}
                  roadName={getRoadName(dir)}
                  phase={sig.phase}
                  countdown={sig.countdown}
                  greenTime={timer.green_time}
                  isActive={sig.phase === 'green'}
                />
              );
            })}
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontFamily: 'Rajdhani', fontSize: '1rem', marginBottom: 5 }}>
              ...
              <span style={{ color: '#15803d', fontFamily: 'Orbitron', fontWeight: 700 }}>
                {getActiveDirection()}
              </span>
            </div>
            <div style={{ fontFamily: 'Rajdhani', fontSize: '1rem' }}>
              <span style={{ color: '#78716c' }}>Cycle Length: </span>
              <span style={{ color: '#1a2744', fontFamily: 'Orbitron', fontWeight: 700 }}>
                {getCycleLength()}s
              </span>
            </div>
          </div>

          {/* Per-direction timer mini-cards with top-border by direction accent */}
              ...
              return (
                <div key={dir} style={{
                  ...
                  <div style={{
                    ...
                    color: isGreen ? '#15803d' : '#1a2744', marginBottom: 4,
                  }}>
        ...
        {/* ─ RIGHT: Vehicle Input + Results ───────────────────────────── */}
        <div
          className="panel panel-input"
          style={{ flex: 1, padding: 20, display: 'flex', flexDirection: 'column', gap: 14, overflow: 'auto' }}
        >
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Rajdhani' }}>
              ...
              <tbody>
                {VEHICLE_TYPES.map((vt, idx) => {
                  const count    = vehicleInputs[activeTab][vt.key] || 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '7px 12px', color: '#78716c' }}>
                        {vt.pcu.toFixed(2)}
                      </td>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
              ...
              <span style={{
                ...
                color: densityColor, letterSpacing: 1,
              }}>
          ...
          {optimizationResult && (
            <div
              className="panel panel-result animate-fade-in"
            ...
            >
              ...
              <div style={{ height: 190, marginTop: 12, marginBottom: 14 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 4, right: 8, left: -16, bottom: 4 }}>
                    ...
                    <XAxis
                      dataKey="direction"
                      tick={{ fill: '#78716c', fontFamily: 'Orbitron', fontSize: 10 }}
                    />
                    <YAxis
                      tick={{ fill: '#78716c', fontFamily: 'Rajdhani', fontSize: 11 }}
                    />
                    <Tooltip
                      contentStyle={{
                        background: '#faf7f2',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {DIRECTIONS.map(dir => {
                  const oldG = optimizationResult.oldTimers.find(t => t.direction === dir)?.green_time || 30;
                  const newG = optimizationResult.newTimers.find(t => t.direction === dir)?.green_time || 30;
                    ...
                    <div key={dir} style={{
                      ...
                      <div style={{ fontFamily: 'Rajdhani', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{
                          ...
                          color: up ? '#15803d' : down ? '#b91c1c' : '#78716c',
                        }}>
